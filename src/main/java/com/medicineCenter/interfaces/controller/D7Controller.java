...
@Controller
@RequestMapping("/d7")
public class D7Controller extends BaseController {
    ...
    @ResponseBody
    public Result createSix(@RequestBody TaskDTO taskDTO, HttpServletRequest request/*,Model model*/){
        Result result = taskService.addseven(taskDTO);
        return result;
    }
    /**
     * 取消任务
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public Page<Experts> viewList(@PathVariable int pageNum, String q, String asc, String desc, HttpServletRequest request/*,Model model*/){
        if(q!=null)
            q = URLDecoder.decode(q);
        return expertsCrudService.getExperts(pageNum, q, asc, desc,"七科");
    }
