...
import com.medicineCenter.model.dto.TaskOneDTO;
...
@Controller
@RequestMapping("/d1")
public class D1Controller extends BaseController {
    ...
    @ResponseBody
    public Result createTwo(@RequestBody TaskOneDTO taskOneDTO, HttpServletRequest request/*,Model model*/){
        Result result = taskService.addOne(taskOneDTO);
        return result;
    }
    /**
     * 取消任务
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public Page<Experts> viewList(@PathVariable int pageNum, String q, String asc, String desc, HttpServletRequest request/*,Model model*/){
        if(q!=null)
            q = URLDecoder.decode(q);
        return expertsCrudService.getExperts(pageNum, q, asc, desc,"一科");
    }
