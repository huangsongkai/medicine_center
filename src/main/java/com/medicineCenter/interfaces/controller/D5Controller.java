...
import com.medicineCenter.model.dto.TaskFiveDTO;
...
@Controller
@RequestMapping("/d5")
public class  D5Controller extends BaseController {
    ...
    @ResponseBody
    public Result createTwo(@RequestBody TaskFiveDTO taskFiveDTO, HttpServletRequest request/*,Model model*/){
        Result result = taskService.addFive(taskFiveDTO);
        return result;
    }
    /**
     * 取消任务
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public Page<Experts> viewList(@PathVariable int pageNum, String q, String asc, String desc, HttpServletRequest request/*,Model model*/){
        if(q!=null)
            q = URLDecoder.decode(q);
        return expertsCrudService.getExperts(pageNum, q, asc, desc,"五科");
    }
