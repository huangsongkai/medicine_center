...
@Controller
@RequestMapping("/d6")
public class D6Controller extends BaseController {
    ...
    @ResponseBody
    public Result createSix(@RequestBody TaskSixDTO taskSixDTO, HttpServletRequest request/*,Model model*/){
        Result result = taskService.addsix(taskSixDTO);
        return result;
    }
    /**
     * 取消任务
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public Page<Experts> viewList(@PathVariable int pageNum, String q, String asc, String desc, HttpServletRequest request/*,Model model*/){
        if(q!=null)
            q = URLDecoder.decode(q);
        return expertsCrudService.getExperts(pageNum, q, asc, desc,"六科");
    }
